...
// jsdom 29.1.1 provides `crypto.getRandomValues` but `crypto.subtle` is `undefined`,
// so real WebCrypto is stubbed in per-test/per-suite rather than in vitest.setup.ts -
// a global stub would make the "subtle unavailable" branch untestable.
describe('oauth-pkce', () => {
  ...
  describe('with getRandomValues stubbed for bias detection', () => {
    ...
    it('produces the exact RANDOM_ALPHABET characters for sequential byte values, proving no modulo bias', () => {
      // Sequential bytes 0,1,2,...,255 wrapping. A `% 66` implementation would
      // produce a provably different string than indexing with `& 63`.
