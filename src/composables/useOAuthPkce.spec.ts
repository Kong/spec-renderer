...
// jsdom provides `crypto.getRandomValues` but not `crypto.subtle` - real WebCrypto is
// stubbed in for the whole file (per-test overrides restore it in their own afterEach),
// matching oauth-pkce.spec.ts.
beforeAll(() => {
...
// jsdom's `window.open` returns `undefined` - stand in a fake pop-up so the flow has
// something to navigate/poll/close.
...
// Plain object satisfying the bits of `Response` this module reads - no need for a real
// `Response` since `fetch` is fully mocked.
...
// jsdom's `window.postMessage` delivers `origin: ''` and `source: null`, which correct
// validation rejects - hand-build the event the callback page would have sent instead.
...
/** Yield one real macrotask - only for cases with no pending crypto-digest await left
 *  to wait out (e.g. confirming nothing further happens after a flow has settled). */
const tick = (): Promise<void> => new Promise(resolve => setTimeout(resolve, 0))

/**
 * Poll until `predicate()` is true, or throw after `timeoutMs`. Used instead of a fixed
 * `setTimeout(0)` tick to wait for the crypto-digest await inside `authorize()` to
 * resolve - a fixed single tick was observed to flake under the CPU load of the full
 * test suite running in parallel, where the real WebCrypto digest can take longer than
 * one macrotask to settle.
 */
...
/**
 * Wait until `authorize()` has navigated the pop-up past `baselineCount` - i.e. past the
 * PKCE-challenge digest for the flow just started.
 *
 * Comparing against a captured baseline (rather than "at least one call exists") matters
 * because `window.open` always returns the very same fake pop-up object across multiple
 * `authorize()` calls within one test: a bare "any calls exist" check would resolve
 * instantly on a *previous* flow's navigation and read stale data (or, worse, dispatch a
 * callback message before the new flow's listener is even attached, hanging the test
 * until the real 5-minute flow timeout).
 */
...
/**
 * Drive `authorize()` for `target` all the way to a committed token, using whatever
 * `global.fetch` is mocked to at call time (the token exchange response).
 */
...
// ─── statusFor / aggregateStatus / tokenFor / errorFor ──────────────────────────
...
// ─── authorize - guard rails before any popup ────────────────────────────────────
...
describe('authorize guard rails', () => {
  ...
  it('opens the popup synchronously, before any await', async () => {
    ...
    const navCountBefore = fakePopup.location.replace.mock.calls.length
    const promise = authorize({ target, ...AUTHORIZE_OPTS_BASE })
    // Same tick, before any `await` in this test - proves `window.open` ran
    // synchronously inside `authorize()`, not after the PKCE-challenge digest.
    expect(window.open).toHaveBeenCalledTimes(1)

    // Let the flow finish installing its real timers/listeners, then tear it down so
...
// ─── authorize - URL correctness (highest-value test) ───────────────────────────
...
// ─── authorize - callback error taxonomy ─────────────────────────────────────────
...
// ─── pop-up lifecycle (closed / timeout) ─────────────────────────────────────────

/**
 * Flush pending microtasks/macrotasks several times over. The real WebCrypto digest
 * awaited inside `authorize()` resolves independently of fake timers, so a single
 * `advanceTimersByTimeAsync(0)` isn't guaranteed to give it enough real wall-clock time
 * to settle under load - looping gives it several chances to do so before we start
 * advancing fake time for the pop-up-poll/timeout assertions below.
 */
