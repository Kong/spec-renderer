...
// Registers with the auth-preflight registry so TryIt.vue can run this alongside
// other flow components' pre-request handlers for the active security scheme group.
const { registerPreflight, unregisterPreflight } = composables.useAuthPreflight()
onMounted(() => registerPreflight(props.schemeKey, auth2ClientCredentialsAuth))
